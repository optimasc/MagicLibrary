...
public class Main {
    ...
    private static void extractRecursive(IdentifierMagicDB extractor, Configuration config, File fs) throws FileNotFoundException, IOException
    {
        ...
        if (fs.isDirectory() == false)
        {
            try
            {
                extractFromResource(extractor, config, fs);
            } catch (IOException e)
            {
                System.err.println("Error: Could not read " + fs.getName() + ", skipping.");
            }
    ...
    private static void processParameters(Configuration config, String[] args)
    {
        ...
        if (args.length == 0)
        {
            ...
            System.out.println("  filespec : File or directory specification identification where and what to scan");
